...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($matricule) && !empty($nom) && !empty($prenom)) {
        try {
            ...
            if ($enseignant_existant) {
                ...
                // Nouvel enseignant : si login/mot de passe non fournis, on les génère automatiquement
                if (empty($login)) {
                    // login par défaut : prenom.nom (sans espaces), en minuscules
                    $login = strtolower(preg_replace('/[^a-z0-9\.]/i', '', $prenom . '.' . $nom));
                }
                ...
                if ($check_login->fetch()) {
                    // si collision, ajouter un suffixe numérique
                    $sfx = 1;
                    $base = $login;
                    while ($check_login->fetch() || $pdo->query("SELECT 1 FROM utilisateur WHERE login = '" . addslashes($login) . "'")->fetch()) {
                        $login = $base . $sfx;
                        $sfx++;
                        if ($sfx > 1000) break;
                    }
                ...
                if (empty($mdp_brut)) {
                    // mot de passe généré lisible et conforme: 10 chars avec majuscule, chiffre et spécial
                    $mdp_brut = 'ChangeMe1!';
                }
                ...
                // Conserver les identifiants générés pour l'affichage
                $info_creden = "Identifiants du compte -> Login: $login | Mot de passe: $mdp_brut";
            ...
            // Nettoyage des anciennes affectations pour éviter les doublons ou conflits
            $pdo->prepare("DELETE FROM affectation_enseignant WHERE id_utilisateur = :id_user")->execute(['id_user' => $id_user]);

            // Traitement des affectations multiples
            $affectations_ajoutees = 0;
            
            foreach ($classes_post as $idx => $classe_item) {
                $matiere_iter = trim($matieres[$idx] ?? '');
            ...
            if ($affectations_ajoutees > 0) {
                $message_succes = "Enseignant et affectations enregistrés avec succès ! ($affectations_ajoutees affectation(s) créée(s))";
            } else {
                $message_succes = "Fiche de l'enseignant enregistrée avec succès.";
            }

            if (!empty($info_creden)) {
                $message_succes .= " -- " . $info_creden;
            }
    ...
    // Gestion propre de la réponse retournée au format JSON pour l'AJAX
...
<head>
    ...
    <style>
        ...
        .checkbox-group { max-height: 180px; overflow-y: auto; border: 1px solid #99c1e9; border-radius: 4px; padding: 10px; background-color: #fff; }
        .checkbox-group .form-check { margin-bottom: 6px; }
...
<div class="container mt-4 mb-4">
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <div id="ajax-response-zone"></div>

            <form id="form-maitre" method="POST" class="needs-validation">
                ...
                <div class="form-section mb-4" id="credentials-section" <?php echo !empty($info_enseignant['matricule']) ? 'style="display:none;"' : ''; ?>>
                    <h5 class="text-secondary mb-3">
...
// ENVOI DU FORMULAIRE VIA AJAX VERS LUI-MÊME
$('#form-maitre').on('submit', function(e){
    ...
    $.ajax({
        type: 'POST',
        url: window.location.href, // FORCE L'ENVOI SUR LA PAGE ACTUELLE
        data: form.serialize(),
        dataType: 'json',
        headers: {
        ...
        success: function(data){
            ...
            if(data.succes){
                ...
                // Si tout a réussi, on vide le formulaire pour une nouvelle insertion
                form[0].reset();
                updateAffectationUI();
            } else {
        ...
        error: function(xhr, status, error){
            console.error(xhr.responseText); // Permet de voir le vrai bug en console si besoin
            zone.removeClass('alert alert-success').addClass('alert alert-danger alert-dismissible fade show').html(
                '<i class="fas fa-exclamation-circle mr-2"></i>Une erreur est survenue lors de l\'enregistrement.' +
                '<button type="button" class="close" data-dismiss="alert"><span>&times;</span></button>'
            );
        }
    });
